nmain <- function(.name, .k, .POP) {
  ...
  TournamentSelection <- function(Population) {
    ...
    for (i in 1:(length(Population) / 2)) {
      tempR <-
        sample(1:length(Population), 2, replace = FALSE) # generate two random indexes of "chromosome" from population
    ...
    repeat {
      if (.parent2[i] %in% .off) {
        ...
        if (choi == T) {
          ...
          count <- count + 1
          if ((i > length(.parent1)) &&
              (count != length(.parent1[(max(.cut) + 1):length(.parent1)]))) {
            i <- 1
            temp <- F
          }
  ...
  recOwn <- function(.population, choi) {
    ...
    repeat {
      ...
      if (choi == "OX") {
        ...
        .toff1 <-
          c(parent2[1:(min(.cut) - 1)], .off1, parent2[(max(.cut) + 1):length(parent2)])
        .toff2 <-
          c(parent1[1:(min(.cut) - 1)], .off2, parent1[(max(.cut) + 1):length(parent2)])
  ...
  Mutation <- function(Population) {
    for (i in 1:length(Population)) {
      .rand <- runif(1, 0, 1)
      if (.rand < p.mut) {
        # check probability of the mutation
        tempR <-
          sample(1:N, 2, replace = F) # generate two random indexes of gene to be swaped
        Population[[i]] <-
          Swap(Population[[i]], tempR) # swap them (mutate)
      } else{
        next
      }
    }
  ...
  Genetic <- function(Population) {
    ...
    final <- list()
    bestP <-
      TournamentSelection(Population) # selection best parents
    if (.rand < p.recomb) {
      # check probability of the recombination
  ...
  main <- function(.FirstPopulation) {
    FinalSchedule <<- NULL
    for (i in 1:k) {
      # generate generations = k, k = stop condition
      if (i == 1) {
        # for the first generation
        .new <-
          Genetic(.FirstPopulation) # run the genetic algorithm
        tempMIN <-
          BestInPopulation(.new) # the best schedule time from the returned population
        dE <- tempMIN - curMIN
        if (dE < 0) {
          # if we found better scheduling time
          curMIN <<-
            tempMIN # override with the new optimal scheduling time
          cat("Currently the best = ",
              curMIN,
              "\n",
              file = "output_PMX.txt",
              append = TRUE)
          ...
          cat(
            "No better solution for the generation = ",
            i,
            "\n",
            file = "output_PMX.txt",
            append = TRUE
        ...
        if (i >= (k * 0.33)) {
          # the point at which we began to change the probability of mutation and recombination
  ...
  cat("Output for the own PMX recombinator",
      ...
      file = "output_PMX.txt",
      append = TRUE)
  cat("Output for the number of iterations = ",
      k,
      "\n",
      file = "output_PMX.txt",
      append = TRUE)
  cat(
    "Output for the number of persons in population = ",
    POP,
    "\n",
    file = "output_PMX.txt",
    append = TRUE
  ...
  FirstPopulation <-
    FirstGenerationOFPopulation() # generate first population
  curMIN <<-
    BestInPopulation(FirstPopulation) # the best schedule time for the first population
  cat(
    "Optimal schedule time for the start population = ",
    curMIN,
    "\n",
    file = "output_PMX.txt",
    append = TRUE
